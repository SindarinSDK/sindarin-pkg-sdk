...
/* Copy callback for Json handles. Called during rt_arena_v2_promote/clone.
 * After the shallow memcpy, the new SnJson copy still references the same
 * json-c object. We must:
 * 1. Increment json-c refcount (old handle's cleanup will decrement it)
 * 2. Update the handle back-reference to point to the new handle
 ...
 * Returns the arena handle (RtHandleV2 *) for proper handle-based promotion.
 * If the object is non-NULL, registers a cleanup callback to release the
 * json-c reference when the arena is destroyed. */
static RtHandleV2 *sn_json_wrap(RtArenaV2 *arena, json_object *obj, int is_root)
{
...
RtHandleV2 *sn_json_parse(RtArenaV2 *arena, const char *text)
{
    if (arena == NULL) {
...
RtHandleV2 *sn_json_parse_file(RtArenaV2 *arena, const char *path)
{
    if (arena == NULL) {
...
RtHandleV2 *sn_json_new_object(RtArenaV2 *arena)
{
    if (arena == NULL) {
...
RtHandleV2 *sn_json_new_array(RtArenaV2 *arena)
{
    if (arena == NULL) {
...
RtHandleV2 *sn_json_new_null(RtArenaV2 *arena)
{
    if (arena == NULL) {
...
RtHandleV2 *sn_json_new_bool(RtArenaV2 *arena, bool value)
{
    if (arena == NULL) {
...
RtHandleV2 *sn_json_new_int(RtArenaV2 *arena, int64_t value)
{
    if (arena == NULL) {
...
RtHandleV2 *sn_json_new_float(RtArenaV2 *arena, double value)
{
    if (arena == NULL) {
...
RtHandleV2 *sn_json_new_string(RtArenaV2 *arena, const char *value)
{
    if (arena == NULL) {
...
bool sn_json_is_object(RtHandleV2 *j)
{
    if (j == NULL) return false;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return false;
    return json_object_is_type(_j->obj, json_type_object);
}

bool sn_json_is_array(RtHandleV2 *j)
{
    if (j == NULL) return false;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return false;
    return json_object_is_type(_j->obj, json_type_array);
}

bool sn_json_is_string(RtHandleV2 *j)
{
    if (j == NULL) return false;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return false;
    return json_object_is_type(_j->obj, json_type_string);
}

bool sn_json_is_number(RtHandleV2 *j)
{
    if (j == NULL) return false;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return false;
    return json_object_is_type(_j->obj, json_type_int)
        || json_object_is_type(_j->obj, json_type_double);
}

bool sn_json_is_int(RtHandleV2 *j)
{
    if (j == NULL) return false;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return false;
...
bool sn_json_is_float(RtHandleV2 *j)
{
    if (j == NULL) return false;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return false;
    return json_object_is_type(_j->obj, json_type_double);
}

bool sn_json_is_bool(RtHandleV2 *j)
{
    if (j == NULL) return false;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return false;
    return json_object_is_type(_j->obj, json_type_boolean);
}

bool sn_json_is_null(RtHandleV2 *j)
{
    if (j == NULL) return true;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return true;
    return json_object_is_type(_j->obj, json_type_null);
}
...
RtHandleV2 *sn_json_as_string(RtArenaV2 *arena, RtHandleV2 *j)
{
    if (j == NULL) return rt_arena_v2_strdup(arena, "");
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) {
        return rt_arena_v2_strdup(arena, "");
    }
    if (!json_object_is_type(_j->obj, json_type_string)) {
        return rt_arena_v2_strdup(arena, "");
    }
    const char *str = json_object_get_string(_j->obj);
    return rt_arena_v2_strdup(arena, str ? str : "");
}

int64_t sn_json_as_int(RtHandleV2 *j)
{
    if (j == NULL) return 0;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return 0;
    if (!json_object_is_type(_j->obj, json_type_int)
        && !json_object_is_type(_j->obj, json_type_double)) return 0;
    return json_object_get_int64(_j->obj);
}

int64_t sn_json_as_long(RtHandleV2 *j)
{
    if (j == NULL) return 0;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return 0;
...
double sn_json_as_float(RtHandleV2 *j)
{
    if (j == NULL) return 0.0;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return 0.0;
    if (!json_object_is_type(_j->obj, json_type_double)
        && !json_object_is_type(_j->obj, json_type_int)) return 0.0;
    return json_object_get_double(_j->obj);
}

bool sn_json_as_bool(RtHandleV2 *j)
{
    if (j == NULL) return false;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return false;
    if (!json_object_is_type(_j->obj, json_type_boolean)) return false;
    return json_object_get_boolean(_j->obj);
}
...
RtHandleV2 *sn_json_get(RtArenaV2 *arena, RtHandleV2 *j, const char *key)
{
    if (j == NULL || key == NULL) {
    ...
    if (!json_object_is_type(_j->obj, json_type_object)) {
        return sn_json_wrap(arena, NULL, 0);
    }

    json_object *val = NULL;
    if (!json_object_object_get_ex(_j->obj, key, &val)) {
        return sn_json_wrap(arena, NULL, 0);
    }
...
bool sn_json_has(RtHandleV2 *j, const char *key)
{
    if (j == NULL || key == NULL) return false;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return false;
    if (!json_object_is_type(_j->obj, json_type_object)) return false;
    return json_object_object_get_ex(_j->obj, key, NULL);
}

RtHandleV2 *sn_json_keys(RtArenaV2 *arena, RtHandleV2 *j)
{
    if (j == NULL) {
        return rt_array_create_string_v2(arena, 0, NULL);
    }
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL || !json_object_is_type(_j->obj, json_type_object)) {
        return rt_array_create_string_v2(arena, 0, NULL);
    }
    ...
    json_object_object_foreachC(_j->obj, iter) {
...
RtHandleV2 *sn_json_get_at(RtArenaV2 *arena, RtHandleV2 *j, int64_t index)
{
    if (j == NULL) {
        return sn_json_wrap(arena, NULL, 0);
    }
    ...
    if (!json_object_is_type(_j->obj, json_type_array)) {
        return sn_json_wrap(arena, NULL, 0);
    }
    size_t len = json_object_array_length(_j->obj);
    if (index < 0 || (size_t)index >= len) {
        return sn_json_wrap(arena, NULL, 0);
    }

    json_object *val = json_object_array_get_idx(_j->obj, (size_t)index);
    /* Increment refcount - child wrapper owns a reference */
    if (val != NULL) {
        json_object_get(val);
    }
...
RtHandleV2 *sn_json_first(RtArenaV2 *arena, RtHandleV2 *j)
{
    if (j == NULL) {
        return sn_json_wrap(arena, NULL, 0);
    }
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL || !json_object_is_type(_j->obj, json_type_array)) {
        return sn_json_wrap(arena, NULL, 0);
    }
    size_t len = json_object_array_length(_j->obj);
    if (len == 0) {
    ...
    json_object *val = json_object_array_get_idx(_j->obj, 0);
    if (val != NULL) {
        json_object_get(val);
    }
    return sn_json_wrap(arena, val, 0);
...
RtHandleV2 *sn_json_last(RtArenaV2 *arena, RtHandleV2 *j)
{
    if (j == NULL) {
        return sn_json_wrap(arena, NULL, 0);
    }
    ...
    json_object *val = json_object_array_get_idx(_j->obj, len - 1);
    if (val != NULL) {
        json_object_get(val);
    }
    return sn_json_wrap(arena, val, 0);
...
int64_t sn_json_length(RtHandleV2 *j)
{
    if (j == NULL) return 0;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return 0;
    ...
    if (json_object_is_type(_j->obj, json_type_object)) {
        return (int64_t)json_object_object_length(_j->obj);
    }
    if (json_object_is_type(_j->obj, json_type_array)) {
        return (int64_t)json_object_array_length(_j->obj);
    }
    return 0;
}
...
void sn_json_set(RtHandleV2 *j, const char *key, RtHandleV2 *value)
{
    ...
    SnJson *_j = (SnJson *)j->ptr;
    SnJson *_value = (SnJson *)value->ptr;
    if (_j->obj == NULL) {
        fprintf(stderr, "Json.set: invalid arguments\n");
        return;
    }
    ...
    if (_value->obj != NULL) {
        if (json_object_deep_copy(_value->obj, &copy, NULL) != 0) {
    ...
    json_object_object_del(_j->obj, key);
    json_object_object_add(_j->obj, key, copy);
}

void sn_json_remove(RtHandleV2 *j, const char *key)
{
    if (j == NULL || key == NULL) return;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return;
    if (!json_object_is_type(_j->obj, json_type_object)) return;
...
void sn_json_append(RtHandleV2 *j, RtHandleV2 *value)
{
    ...
    json_object_array_add(_j->obj, copy);
}

void sn_json_prepend(RtHandleV2 *j, RtHandleV2 *value)
{
    if (j == NULL || value == NULL) {
    ...
    json_object_array_insert_idx(_j->obj, 0, copy);
}

void sn_json_insert(RtHandleV2 *j, int64_t index, RtHandleV2 *value)
{
    if (j == NULL || value == NULL) {
    ...
    json_object_array_insert_idx(_j->obj, (size_t)index, copy);
}

void sn_json_remove_at(RtHandleV2 *j, int64_t index)
{
    if (j == NULL) return;
    SnJson *_j = (SnJson *)j->ptr;
    if (_j->obj == NULL) return;
    if (!json_object_is_type(_j->obj, json_type_array)) return;
    size_t len = json_object_array_length(_j->obj);
    ...
    json_object_array_del_idx(_j->obj, (size_t)index, 1);
}
...
RtHandleV2 *sn_json_to_string(RtArenaV2 *arena, RtHandleV2 *j)
{
    if (j == NULL) {
    ...
    const char *str = json_object_to_json_string_ext(_j->obj, JSON_C_TO_STRING_PLAIN);
    if (str == NULL) {
        return rt_arena_v2_strdup(arena, "null");
    }
...
RtHandleV2 *sn_json_to_pretty_string(RtArenaV2 *arena, RtHandleV2 *j)
{
    if (j == NULL) {
        return rt_arena_v2_strdup(arena, "null");
    }
    ...
    const char *str = json_object_to_json_string_ext(_j->obj, JSON_C_TO_STRING_PRETTY);
    if (str == NULL) {
        return rt_arena_v2_strdup(arena, "null");
    }
...
void sn_json_write_file(RtHandleV2 *j, const char *path)
{
    if (j == NULL || path == NULL) {
    ...
    if (json_object_to_file_ext(path, _j->obj, JSON_C_TO_STRING_PLAIN) != 0) {
        const char *err = json_util_get_last_err();
...
void sn_json_write_file_pretty(RtHandleV2 *j, const char *path)
{
    if (j == NULL || path == NULL) {
    ...
    if (json_object_to_file_ext(path, _j->obj, JSON_C_TO_STRING_PRETTY) != 0) {
        const char *err = json_util_get_last_err();
...
RtHandleV2 *sn_json_copy(RtArenaV2 *arena, RtHandleV2 *j)
{
    if (j == NULL) {
    ...
    json_object *copy = NULL;
    if (json_object_deep_copy(_j->obj, &copy, NULL) != 0) {
...
RtHandleV2 *sn_json_type_name(RtArenaV2 *arena, RtHandleV2 *j)
{
    if (j == NULL) {
        return rt_arena_v2_strdup(arena, "null");
    }
    ...
    if (json_object_is_type(_j->obj, json_type_object))  return rt_arena_v2_strdup(arena, "object");
    if (json_object_is_type(_j->obj, json_type_array))   return rt_arena_v2_strdup(arena, "array");
    if (json_object_is_type(_j->obj, json_type_string))  return rt_arena_v2_strdup(arena, "string");
    if (json_object_is_type(_j->obj, json_type_boolean)) return rt_arena_v2_strdup(arena, "bool");
    if (json_object_is_type(_j->obj, json_type_null))    return rt_arena_v2_strdup(arena, "null");
    if (json_object_is_type(_j->obj, json_type_int))     return rt_arena_v2_strdup(arena, "number");
    if (json_object_is_type(_j->obj, json_type_double))  return rt_arena_v2_strdup(arena, "number");
...
void sn_json_dispose(RtHandleV2 *j)
{
    if (j == NULL) return;
    SnJson *_j = (SnJson *)j->ptr;

    if (_j->obj != NULL) {
        json_object_put(_j->obj);
        _j->obj = NULL;
    }
